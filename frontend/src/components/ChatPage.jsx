...
const ChatPage = () => {
    ...
    const sendMessageHandler = async (receiverId) => {
        try {
            const res = await axios.post(`https://igrams.onrender.com/api/v1/message/send/${receiverId}`, { textMessage }, {
                headers: {
    ...
    return (
        <div className='flex ml-[16%] h-screen'>
            {/* Only show the suggested users list if no user is selected */}
            {!selectedUser && (
                ...
                selectedUser ? (
                    <section className='flex-1 border-l border-l-gray-300 flex flex-col h-full'>
                        <div className='flex gap-3 items-center px-3 py-2 border-b border-gray-300 sticky top-0 bg-white z-10'>
                            {/* Back Button */}
                            <ArrowLeft onClick={() => dispatch(setSelectedUser(null))} className='cursor-pointer' />
                            <Avatar>
